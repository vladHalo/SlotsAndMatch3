...
public static class MathfsExtensions {
	...
	public static float RemapClamped( this float v, float iMin, float iMax, float oMin, float oMax ) => Mathfs.RemapClamped( iMin, iMax, oMin, oMax, v );
	public static float Repeat( this float v, float length ) => Mathfs.Repeat( v, length );
	public static double Repeat( this double t, double length ) => Mathfs.Clamp(t - Math.Floor(t / length) * length, 0.0f, length);
